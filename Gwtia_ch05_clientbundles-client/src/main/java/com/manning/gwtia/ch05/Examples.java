package com.manning.gwtia.ch05;
...
public class Examples implements EntryPoint, ValueChangeHandler<String> {
	...
	/**
	 * Entry Point. Add an ExamplePanel to the RootPanel, and handle any history
	 * token that might have been present when application started (this might lead
	 * to starting an example immediately, or leaving the introduction screen
	 * displayed).
	 */
	...
	/**
	 * Register this class as handling history events, and check to see if we
	 ...
	 * 
	 * If there is a change to the history, then a ValueChangeEvent is fired. This
	 * method handles that by parsing the found token, and based on the value, it
	 * can request the examplePanel shows the History example, Layout example, or
